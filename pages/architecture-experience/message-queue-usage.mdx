...
RabbitMQ `data-live-queue` 의 데이터를 수신하는 작업은 데이터의 트래픽이 높기에 별도의 스레딩 처리가 필요했습니다. 이렇게 리스너에서 데이터를 수신하는 역할을 하는 `listenerExecutor` 라는 별도의 ExecutorService 에 별도로 선언해두었고 데이터의 수신을 모두 람다 바디 안에서 이뤄지게끔 했습니다.<br/>
...
실시간 트래픽이 높고, 데이터의 수정이 자주 발생하는 증권데이터의 특성을 고려해서 Outbox 로 사용할 데이터 스토리지을 결정해야 하는데, 데이터의 수정이 빈번할 경우 관계형 DB의 경우 MVCC 로 인한 이슈가 발생합니다. 따라서 작업 데이터의 대기열 용도의 Outbox 로 사용할 데이터베이스를 지금 시점에서 이 부분에 대한 결정을 내린다면, MongoDB 또는 influxDB와 같은 NoSQL 기반의 데이터베이스의 컬렉션을 두었을 것 같습니다.<br/>
